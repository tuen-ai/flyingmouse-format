...
- 离线单文件网页版「转档台 FormatDeck」（繁体中文界面）：`offline/dist/formatdeck-offline.html` 一个文件（约 171 KB）、双击用浏览器打开就能用，免安装、免联网、无服务端，支持图片互转 / 图片合并 PDF / 文本与表格互转 / EPUB / DOCX / 打包 ZIP（详见 [docs/离线单文件工具.md](docs/离线单文件工具.md)）。
...
### 免安装：离线单文件网页版「转档台 FormatDeck」

![离线单文件网页版界面](public/assets/screenshots/offline-home.png)

装不了桌面版（公司电脑、Mac、临时用一次）时，用离线单文件版：

1. 取仓库里的 `offline/dist/formatdeck-offline.html`（约 171 KB，一个文件就是全部）。
...
- Offline single-file web build "FormatDeck" (Traditional Chinese UI): `offline/dist/formatdeck-offline.html` (~171 KB) runs in any browser with no install, no server and no network — image conversion, images-to-PDF, text/table conversion, EPUB, DOCX and ZIP packaging (see [docs/离线单文件工具.md](docs/离线单文件工具.md)).
...
### No install: the offline single-file build (FormatDeck)

When the desktop app cannot be installed (locked-down machine, Mac, one-off job), use the single HTML file:

1. Grab `offline/dist/formatdeck-offline.html` from this repository (~171 KB, everything is inlined).
